...
// --- EXPORT 1: The Magic Container ---
export const MagicContainer: React.FC<{ children: React.ReactNode; className?: string; enableSpotlight?: boolean; spotlightRadius?: number; glowColor?: string; }> = ({ children, className = '', enableSpotlight = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }) => {
  ...
  return (
    <div className={`bento-section ${className}`} ref={gridRef}>
      {enableSpotlight && <GlobalSpotlight gridRef={gridRef} disableAnimations={isMobile} enabled={enableSpotlight} spotlightRadius={spotlightRadius} glowColor={glowColor} />}
      {children}
    </div>
  );
};

// --- EXPORT 2: The Magic Card (Wraps your content) ---
export const MagicCard: React.FC<{ children: React.ReactNode; className?: string; enableStars?: boolean; particleCount?: number; glowColor?: string; enableTilt?: boolean; clickEffect?: boolean; enableMagnetism?: boolean; }> = ({ children, className = '', enableStars = true, particleCount = DEFAULT_PARTICLE_COUNT, glowColor = DEFAULT_GLOW_COLOR, enableTilt = false, clickEffect = true, enableMagnetism = false }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (enableMagnetism) gsap.to(element, { x: (x - centerX) * 0.05, y: (y - centerY) * 0.05, duration: 0.3, ease: 'power2.out' });
    };
  ...
  }, [animateParticles, clearAllParticles, enableTilt, enableMagnetism, clickEffect, enableStars, glowColor]);

  // THE FIX: Adding 'magic-bento-card' to the className ensures the GlobalSpotlight CSS attaches to it!
  return (
    <div ref={cardRef} className={`magic-bento-card magic-bento-card--border-glow ${className}`} style={{ position: 'relative', overflow: 'hidden', '--glow-color': glowColor } as React.CSSProperties}>
      {children}
    </div>
  );
};
...
// ... ALL YOUR POLYGON MATH FUNCTIONS ...
...
function getThemeTransitionClipPaths(
  variant: TransitionVariant, cx: number, cy: number, maxRadius: number, viewportWidth: number, viewportHeight: number
): [string, string] {
  switch (variant) {
    case "circle":
...
export const AnimatedThemeToggler = ({
  className,
  duration = 500, // Slightly longer for a smoother paint effect
  variant = "circle",
  fromCenter = false,
  ...props
    ...
    if (ready && typeof ready.then === "function") {
      ready.then(() => {
        document.documentElement.animate(
          ...
          {
            duration,
            easing: variant === "star" ? "linear" : "cubic-bezier(0.64, 0, 0.78, 0)",
            fill: "forwards",
  ...
  return (
    <button
      ...
      ref={buttonRef}
      onClick={toggleTheme}
      className={cn("relative flex items-center justify-center rounded-full text-slate-800 dark:text-white transition-transform hover:scale-110", className)}
      {...props}
    >
